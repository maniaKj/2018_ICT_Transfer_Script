...
#pragma target 3.0
	...
	UNITY_INSTANCING_BUFFER_START(Props)
		...
		float time_line = step(0.6, frac(_Time.y));
		outline = (step(time_factor, 0.5)*time_factor + step(1 - time_factor, 0.5) * (1 - time_factor)) * _NoiseWidth + _NoiseBias;
		float tmp = step(outline, mask.r);
		//o.Albedo = c.rgb * tmp + grid.rgb * (1- tmp);
		//o.Emission = _GridColor * (1 - tmp) * time_line;
		o.Albedo = c.rgb*(1 - tmp * time_line);
		o.Alpha = c.a;
	}
	ENDCG
	}

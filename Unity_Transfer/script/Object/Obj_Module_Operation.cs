...
public class Obj_Module_Operation : MonoBehaviour {
    ...
    //private
    Module_Lable[] child_Module_script;
    ...
    void Awake() {
        ...
        child_Module_script = new Module_Lable[child_module_Num];
        for (int i = 0; i < child_module_Num; i++)
        {
            child_Module[i] = temp_Modules[i];
            child_Module_script[i] = child_Module[i].GetComponent<Module_Lable>();
            child_Module_script[i].MOP = this;
        ...
        foreach (int opID in module_Operation_Array)
        {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                else
                {
                    ...
                    {
                        if(child_Module_script[j].ModuleID == Correct_Code_Part[i] && OverCheck[j] == false)
                        {
                            tmp[i] = true;
                            OverCheck[j] = true;
                            break;
    ...
    public void Get_Child_Signal(int moduleID)
    {
    ...
    void Module_ID_Change()
    {
        ...
        foreach (GameObject module in child_Module)
        {
            if (GetComponent<Puzzle_Part_Hologram>() != null)
            {
                ...
                module.GetComponent<Module_Lable>().ModuleID += i * 100 + sectionID * 1000 + 10000;
            }
            i++;
        }
    }//모듈 특성 많아지면 여길 활용할 것
    ...
    void Find_Module_Obj(GameObject target) //자식 오브젝트 찾기 + 인식처리되는 모듈에 한해서
    {
        ...
        {
            if (temp.GetChild(i).GetComponent<Module_Lable>() != null)
            {
                if (temp.GetChild(i).GetComponent<Module_Lable>().Operative_Module)
                {

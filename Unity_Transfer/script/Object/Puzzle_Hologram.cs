...
public class Puzzle_Hologram : MonoBehaviour {
    ...
    void Hologram_Assembling(Puzzle_Part_Hologram.Assembling_Data receive_Data)
    {
        ...
        receive_Data.Data_Sender_Obj.GetComponent<Puzzle_Part_Hologram>().Object_Linking(puzzle_Section_pivot[target_Num].transform.GetChild(0).gameObject, receive_Data.linked_Module_Obj.GetComponent<Module_Lable>().ModuleID / 100 % 10);
    ...
    public void Hologram_positioning(int target_Num, Vector3 target_position, Quaternion target_rotation, GameObject linked_Module)
    {
        ...
        if (linked_Module.GetComponent<Module_Lable>().ModuleID >= 20000) puzzle_Section_pivot[target_Num].transform.GetChild(0).localScale = script_PPH.original_Scale * script_PPH.scale_Muliply;
        else if(linked_Module.GetComponent<Module_Lable>().ModuleID < 20000) puzzle_Section_pivot[target_Num].transform.GetChild(0).localScale = script_PPH.original_Scale;
        linked_Module.GetComponent<Module_Lable>().Get_Enabling_Signal(false);
    } // 홀로그램 위치시키기
    ...
    IEnumerator Node_Check_Routine()
    {
        while (true)
        {
            ...
            foreach (GameObject module in Module_Group) if (module.GetComponent<Module_Lable>().ModuleID % 100 == 0) module_assemble_Check = false;
    ...
    void Find_All_Child_Module(GameObject target)
    {
        ...
        for (int i = 0; i < temp.childCount; i++)
        {
            if (temp.GetChild(i).GetComponent<Module_Lable>() != null) Module_Group.Add(temp.GetChild(i).gameObject);

...
public class Puzzle_Hologram_Repository_2 : MonoBehaviour {
    ...
    public void Get_Module_Signal(int get_moduleID)
    {
        ...
        if (get_moduleID == 0)
        {
            ...
            moduleObj.GetComponent<TransferableDataObject>().OnDataTransfer(0);
            Data_Hologram = null;
            GameObject.FindGameObjectWithTag("player").GetComponent<player_shot>().previous_script_ML = moduleObj.GetComponent<TransferableDataObject>();
    ...
    public void Get_Exception_Signal(GameObject Target_Obj)
    {
        ...
        moduleObj.SetActive(true);
        moduleObj.GetComponent<TransferableDataObject>().OnDataTransfer(Target_Obj.GetComponent<HologramObject>().ID);
        Data_Hologram = null;

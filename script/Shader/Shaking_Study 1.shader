Shader "Custom/Shaking_Study 1" {
	...
	_Glossiness("Smoothness", Range(0,1)) = 0.5
		...
		_VertexHeight("Vertex Height",float) =0.0
	}
		SubShader{
...
#pragma target 3.0
	...
	void vert(inout appdata_full v) {
		...
		float tmp3 = frac(_Time.y*_MoveSpeed) * 2 - 1.0;
		//v.vertex.x += _VertexAmount * v.texcoord.x * tmp * step(0.85, tmp);
		//v.vertex.xyz += _VertexAmount * v.normal.xyz * tmp * step(0.5, tmp);
		float tmp2 = step(v.vertex.z, v.vertex.z * tmp);
		tmp2 = step(v.vertex.y, _VertexHeight * (tmp3));
		//if (v.vertex.y < _VertexHeight * (tmp3)) tmp2 = 1; else tmp2 = 0;
		v.vertex.x += _VertexAmount * tmp2;
		//v.vertex.xyz += _VertexAmount * float3(step(0.9, frac(v.vertex.x)), step(0.9, frac(v.vertex.y)), step(0.9, frac(v.vertex.z))) * tmp * step(0.5, tmp);
	}
